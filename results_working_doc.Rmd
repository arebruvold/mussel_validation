...
ggsave(plot_selectivity, filename = "plot_selectivity.pdf", width = 15, height = 5 )
```
...
$$c_m = \frac{\sum_{i=1}^nA_{p_i} / S_{}}{t_{meas}\cdot Q_{det}} = \frac{ C_{std} \sum_{i=1}^nA_{p_i}}{t_{meas} \cdot b}, $$

and the particle number concentration, $c_n$, by

$$c_{n} = \frac{n_{meas}}{t_{meas}\cdot Q_{det}}.$$

Inserting for $Q_{det}$, we get

$$ c_{n}  = \frac{n_{meas} \cdot 6 A_{rm} \cdot C_{std}}{t_{meas} \cdot \rho  \cdot \pi \cdot d_{rm}^3 \cdot b} $$

and for mean particle diameter:

$$
\bar{d}=\frac{1}{n} \sum_{i=1}^n d_{i} = \frac{1}{n} \sum_{i=1}^n \sqrt[3]{\frac{6 A_{i}}{S \pi \rho}} = \frac{1}{n} \sum_{i=1}^n \sqrt[3]{\frac{A_{i}\cdot \rho_{rm} \cdot d_{rm}^3}{A_{rm} \cdot \rho}} \;  .
$$
\frac{6 A_{rm}}{\rho  \cdot \pi \cdot d_{rm}^3}

-   $n_{meas}$ is the instrument measured number of particle events. This corresponds to a Poisson process and hence the standard uncertainty will be approximately
...
For each variable the standard uncertainty is assumed for a Monte Carlo simulation with $10^6$ repeats, reporting the uncertainty with a coverage factor of two. Note that for particles of other elements than gold, the additional uncertainty for calculating from using another ionic standard applies:

$$S_i=S \frac{C_{std}\cdot b_i}{C_i \cdot b}$$

where $S_i$ is the signal per particle mass for element $i$, $C_i$ is the concentration of the ionic standard for element $i$ and $b_i$ is the slope coefficient of the calibration curve for element $i$.
...
poster_processed %>%
  ...
  geom_hline(
    ...
    stat_summary(geom = "errorbar", fun.data = mean_cl_normal, color = "black", fill = "black") +
...
ggsave("poster_mass_conc.pdf", height = 4, width = 9)
...
ggsave("poster_particle_conc.pdf", height = 4, width = 9)
...
```{r}
# set based on observed peak widths
peak_width <- 6
...
# size table
poster_precision_table <- poster_precision %>% filter(parameter == "mean_size") %>% select(isotope, sample_group, isotope, grand_mean, RSE_repeatability,RSE_intermediate_precision, rsd, detection_limit) %>% 
...
```{r poster-QC}
poster_recovery <- poster_processed %>% 
  filter(
    isotope == "Ti",
    str_detect(
      sample_name, "SPIKE TiO2, CuO|853"
    )
  ) %>%
  mutate(
    recovery_group = case_when(
      str_detect(sample_name, "SPIKE") ~ "UPW-S",
      str_detect(sample_name, "\\wS\\s") ~ "BM-S",
      str_detect(sample_name, "[ABC]{1}\\s") ~ "BM",
      TRUE ~ NA
    )
  ) %>% filter(!is.na(recovery_group)) %>% 

  group_by(recovery_group, day) %>% 
  summarise(across(c(mass_conc, particle_conc), mean)) %>% 
  group_by(day) %>% 
  mutate(
    mass_conc = if_else(recovery_group == "BM-S", mass_conc - mass_conc[recovery_group == "BM"], mass_conc)
  )


d1_rec <- subset(poster_recovery, recovery_group == "BM-S" & day == 1)$mass_conc / subset(poster_recovery, recovery_group == "UPW-S" & day == 1)$mass_conc

d2_rec <- subset(poster_recovery, recovery_group == "BM-S" & day == 1)$mass_conc / subset(poster_recovery, recovery_group == "UPW-S" & day == 2)$mass_conc

poster_processed %>%
  filter(sample_name %>% str_detect("PrBlank\\s\\d")) %>%
  group_by(day, isotope) %>%
  summarise(across(
    c(mass_conc, particle_conc, mean_size),
    list(
      mean = ~ mean(., na.rm = T)
    )
  )) %>% 
  mutate(
    ...
    mass_conc_recovery = 
      case_when(
        day == 1 & isotope == "Ti" ~ d1_rec,
        day == 2 & isotope == "Ti" ~ d2_rec,
        TRUE ~ NA
        
      )
  ) %>% 
  select(day, isotope, mass_conc_mean, mass_conc_recovery, particle_conc_mean, n_blanks) %>% 
  gt(groupname_col = "day") %>% 
  fmt_percent(columns = mass_conc_recovery, decimals = 0) %>% 
  fmt_number(
    mass_conc_mean, decimals = 1
  ) %>% 
  fmt_number(
    particle_conc_mean, decimals =  0
  )
...
ggsave(cont_plot, filename = "cont_plot.pdf", width = 10, height = 5.25 )
...
ggsave(filename = "artifat_au_sizes.pdf", width = 15, height = 18)
